  ...
  recursiveUtil.resolveSchema = (sub, index, rootPath) => {
    ...
    if (typeof sub.$ref === "string") {
      ...
      if (
        sub.$ref === "#" ||
        seenRefs[sub.$ref] < 0 ||
        (lastRef === sub.$ref && ++depth > maxDepth)
      ) {
        delete sub.$ref;
        return sub;
      }
    ...
    if (Array.isArray(sub.allOf)) {
      ...
      schemas.forEach((subSchema) => {
        ...
        utils.merge(
          sub,
          typeof _sub.thunk === "function" ? _sub.thunk(sub) : _sub
        );
        if (Array.isArray(sub.allOf)) {
          recursiveUtil.resolveSchema(sub, index, rootPath);
        }
          ...
          mix.forEach((omit) => {
            if (omit.required && omit !== fixed) {
              omit.required.forEach((key) => {
                const includesKey =
                  copy.required && copy.required.includes(key);
    ...
    Object.keys(sub).forEach((prop) => {
      if (
        ...
        sub[prop] = recursiveUtil.resolveSchema(
          sub[prop],
          prop,
