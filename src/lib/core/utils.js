...
function getLocalRef(obj, path) {
  ...
  while (keyElements.length) {
    ...
    if (!obj[prop]) {
      throw new Error(
        `Prop '${prop}' not found in [${Object.keys(obj).join(", ")}] (${path})`
      );
    }
...
function validateValueForOneOf(value, oneOf) {
  const validCount = oneOf.reduce(
    (count, schema) => count + (validateValueForSchema(value, schema) ? 1 : 0),
    0
  );
  return validCount === 1;
}
...
function shouldClean(key, schema) {
  ...
  const wasCleaned =
    ...
    (schema.additionalProperties &&
      typeof schema.additionalProperties.thunk === "function");
