...
function objectType(value, path, resolve, traverseCallback) {
  ...
  const optionalProperties = propertyKeys
    .concat(patternPropertyKeys)
    .reduce((_response, _key) => {
  ...
  const max =
    value.maxProperties ||
    allProperties.length + (allowsAdditional ? random.number(1, 5) : 0);
  ...
  const extraPropertiesRandomOrder = random
    .shuffle(optionalProperties)
  ...
  const _props = requiredProperties.concat(
    random.shuffle(extraProperties).slice(0, _limit)
  );
  const _defns = [];
  ...
  _props.forEach((key) => {
    for (let i = 0; i < ignoreProperties.length; i += 1) {
      if (
        (ignoreProperties[i] instanceof RegExp &&
          ignoreProperties[i].test(key)) ||
        (typeof ignoreProperties[i] === "string" &&
          ignoreProperties[i] === key) ||
        (typeof ignoreProperties[i] === "function" &&
          ignoreProperties[i](properties[key], key))
      ) {
        skipped.push(key);
        return;
      }
    ...
    if (!found) {
      ...
      if (subschema && additionalProperties !== false) {
        ...
        props[patternProperties[key] ? random.randexp(key) : key] =
  ...
  const hash = (suffix) =>
    random.randexp(`_?[_a-f\\d]{1,3}${suffix ? "\\$?" : ""}`);
  ...
  if (allowsAdditional && !requiredProperties.length) {
    minProps = Math.max(
      optionalsProbability === null || additionalProperties
        ? random.number(fillProps ? 1 : 0, max)
        : 0,
      min
    );
  }
  ...
  if (
    ...
    (!allowsAdditional || optionalsProbability === false)
  ) {
